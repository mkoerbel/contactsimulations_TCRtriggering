# Set input folder, calculate from simulation output the phosphorylated TCR lifetimes
...
def calculate_lifetime_5P(file, folder, columnnames_tcrs, columnnames, r_sim, r_c, A_c, A_out, dt, min_lifetime):
    ...
    results.to_json(folder/file.replace('.txt', '_tcr_lifetimes_5P_{}.json'.format(min_lifetime)))

if __name__ == '__main__':
     
    # User input
    folder = '/Users/.../Model_2_5Phos_scan-k/' # results_folder
    ...
    for ifile in listdir(folder):
        # The following arguments can be edited to exclude/include certain files. _tcr_lifetimes.json should always be excluded
        if ('tcrs' in ifile) or ('tcr_lifetimes' in ifile) or ('cd45.txt' in ifile): # only need to find the first .txt file, the others will be loaded
            continue
        elif Path(folder/ifile.replace('.txt', '_tcr_lifetimes_5P_{}.json'.format(min_lifetime))).exists(): # comment if lifetime analysis should be rerun
            continue
        elif ('model_2' in ifile):
